...
## Update Summary
**Changes Made**
- Enhanced questionnaire filling system with comprehensive grouped displays and unified submission capabilities
- Implemented advanced autosave functionality with draft persistence across navigation
- Added sophisticated validation system with real-time error highlighting and global validation
- Introduced single question mode for improved user experience
...
## Introduction
This document describes the evaluation and assessment workflow system, covering the end-to-end lifecycle from questionnaire assignment to completion, scoring, and analytics. The system has been comprehensively enhanced with grouped displays, autosave functionality, unified submission, and advanced validation systems. It explains how different user roles (teacher, staff, parent) access dashboards, how the enhanced questionnaire filling interface works with grouped displays and autosave, and how scoring and result aggregation are performed. It also provides scenario-based walkthroughs of typical user interactions with the improved workflow.

## Project Structure
The evaluation workflow spans routing, middleware-driven role gating, Livewire components for dashboards and forms, Eloquent models for domain entities, and services for scoring and analytics. The routes define the entry points for evaluators and administrators, while middleware ensures only authorized users can access evaluator pages. The system now features enhanced grouped displays, autosave functionality, and unified submission processes.
...
subgraph "Enhanced Forms"
F1["AvailableQuestionnaires.php<br/>Grouped Displays + Unified Submission"]
F2["QuestionnaireFill.php<br/>Autosave + Advanced Validation"]
end
...
- [app/Livewire/Fill/AvailableQuestionnaires.php:1-568](file://app/Livewire/Fill/AvailableQuestionnaires.php#L1-L568)
...
## Core Components
- **Enhanced Routing and role redirection**: Routes under the evaluator prefix and role redirect middleware guide users to appropriate dashboards with improved grouping capabilities.
...
- **Comprehensive Questionnaire assignment and history**: Users see active questionnaires targeting their role (and aliases), draft and submitted histories with enhanced grouping.
- **Advanced Questionnaire filling**: Interactive form with grouped displays, navigation, autosave on transitions, validation per question type, and final submission with unified processing.
...
Dash-->>Browser : Render dashboard with grouped displays
Browser->>Groups : GET /fill/questionnaires
Groups->>Models : Load grouped questionnaires by target groups
...
- [app/Livewire/Fill/AvailableQuestionnaires.php:294-360](file://app/Livewire/Fill/AvailableQuestionnaires.php#L294-L360)
...
### Enhanced Dashboard Views by Role
Each evaluator dashboard computes the same metrics for a specific role slug with improved grouping:
...
- **Grouped displays**: questionnaires are now organized by target groups for better navigation.
...
BuildPayload --> GroupDisplay["Organize by target groups for display"]
GroupDisplay --> End(["Render enhanced dashboard"])
```
...
- **Enhanced active questionnaires**: visible to the user are filtered by questionnaire targets aligned with the user's role and configured aliases with grouping support.
...
- **Grouped organization**: questionnaires are organized by target groups for better user experience.
...
The filling component orchestrates comprehensive features:
- **Initialization**: validates login, active status, and role targeting; loads questions and existing draft answers.
- **Grouped displays**: supports both single questionnaire mode and grouped questionnaire mode with step navigation.
...
### Advanced Grouped Questionnaire System
The enhanced system provides comprehensive grouped questionnaire management:
- **Group organization**: questionnaires are grouped by target groups (e.g., guru_staf, komite) for better navigation.
- **Step navigation**: users can navigate between groups with visual indicators and progress tracking.
- **Unified submission**: allows submitting all questionnaires in a group with single confirmation.
- **Global validation**: validates all required questions across multiple questionnaires before submission.
- **Draft persistence**: automatically saves drafts when navigating between groups or questions.

```mermaid
flowchart TD
A["Load Grouped Questionnaires"] --> B["Organize by Target Groups"]
B --> C["Initialize Questionnaire Metadata"]
C --> D["Load All Answers"]
D --> E["Display Current Group"]
E --> F{"Navigation Action?"}
F --> |Next Group| G["Persist Dirty Drafts"]
G --> H["Load Next Group Questions"]
H --> E
F --> |Previous Group| I["Persist Dirty Drafts"]
I --> J["Load Previous Group Questions"]
J --> E
F --> |Go to Question| K["Navigate to Specific Question"]
K --> E
F --> |Submit All| L["Validate All Required"]
L --> M["Submit All Questionnaires"]
M --> N["Show Success Message"]
```

**Diagram sources**
- [app/Livewire/Fill/AvailableQuestionnaires.php:294-360](file://app/Livewire/Fill/AvailableQuestionnaires.php#L294-L360)
- [app/Livewire/Fill/AvailableQuestionnaires.php:134-165](file://app/Livewire/Fill/AvailableQuestionnaires.php#L134-L165)
- [app/Livewire/Fill/AvailableQuestionnaires.php:178-269](file://app/Livewire/Fill/AvailableQuestionnaires.php#L178-L269)

**Section sources**
- [app/Livewire/Fill/AvailableQuestionnaires.php:1-568](file://app/Livewire/Fill/AvailableQuestionnaires.php#L1-L568)
- [resources/views/livewire/fill/available-questionnaires.blade.php:104-150](file://resources/views/livewire/fill/available-questionnaires.blade.php#L104-L150)
- [resources/views/livewire/fill/available-questionnaires.blade.php:168-360](file://resources/views/livewire/fill/available-questionnaires.blade.php#L168-L360)
...
## Dependency Analysis
The following diagram highlights key dependencies among components involved in the enhanced evaluation workflow.
...
+render() void
+loadGroupedQuestionnaires() void
...
- **Optimized grouped queries**: Summary computations use grouped selects and joins with better indexing on frequently filtered columns (e.g., responses.status, answers.calculated_score).
...
- **Memory optimization**: Grouped questionnaire loading optimizes memory usage by loading questions on-demand rather than all at once.
...
- **Group navigation issues**:
  - Ensure all required questions in the current group are filled before moving to next group.
- **Unified submission problems**:
  - Use the global validation to ensure all required questions across all questionnaires are filled before submission.
...
- [app/Livewire/Fill/AvailableQuestionnaires.php:178-183](file://app/Livewire/Fill/AvailableQuestionnaires.php#L178-L183)
- [app/Livewire/Fill/AvailableQuestionnaires.php:514-554](file://app/Livewire/Fill/AvailableQuestionnaires.php#L514-L554)

## Conclusion
The enhanced evaluation workflow integrates role-aware routing, robust middleware, and Livewire-driven dashboards and forms with comprehensive grouping, autosave, and unified submission capabilities. It enforces assignment rules, supports guided navigation with autosave, validates inputs per question type, and computes accurate scores and analytics. The system now provides enhanced user experience through grouped displays, real-time validation, and streamlined submission processes. Administrators can export consolidated reports for deeper insights with improved analytics.

## Appendices

### Enhanced Workflow Scenarios and User Interactions
- **Teacher dashboard**:
  - Loads available and completed questionnaires based on the teacher role slug and aliases; shows summary statistics with grouped organization.
- **Staff dashboard**:
  - Mirrors teacher dashboard for staff role with enhanced grouping capabilities.
- **Parent dashboard**:
  - Mirrors teacher dashboard for parent role with improved navigation.
- **Enhanced questionnaire filling**:
  - User opens an available questionnaire, navigates through questions with grouped displays, autosaves on transitions, reviews progress, and submits when ready.
- **Unified group submission**:
  - User can navigate between groups, validate all required questions globally, and submit all questionnaires with single confirmation.
