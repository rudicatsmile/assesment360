...
## Introduction
This document explains the role-based access control (RBAC) system used in the application. It covers how roles are defined and stored, how permissions are enforced via middleware and policies, how user-role assignments work, and how department-based access influences questionnaire targeting. Practical examples demonstrate role definitions, permission checks, and access control patterns. Security considerations and best practices for managing roles are included to help maintain a robust and secure authorization model.
...
- Dashboard redirection uses configured paths per role.
...
The RBAC architecture integrates configuration-driven role definitions with runtime enforcement through middleware and policies. Users carry role information, enabling role checks and access decisions. Middleware guards protect routes, while policies govern object-level actions.
